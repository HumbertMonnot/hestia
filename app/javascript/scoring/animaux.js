...
export default async (hexagon) => {
  ...
  const dist = 3
  const url = `${base_url}coords=${hexagon.geometry.coordinates[0][3][0]},${hexagon.geometry.coordinates[0][3][1]}&dist=${dist}&type=animaux`
  ...
  return hexagon.properties.animaux = await turf.distance(my_variable, nearest)
}
